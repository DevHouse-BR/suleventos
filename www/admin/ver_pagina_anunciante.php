...
include("funcoes.php");
$codigo = $_GET["cd"];
$cd_pagina = verifica_cd_pagina_anunciante($codigo);
?>
<html>
	<head>
		...
		function apagar_pagina(codigo){
			if (window.showModalDialog('confirmacao.html',['Confirme!','Deseja apagar esta pagina e todas as suas fotografias?','Sim','Não'],'dialogWidth:320px;dialogHeight:100px;status:no;') == "1"){
				void window.open('delete.php?oque=pagina_anunciante&cd=' + codigo, 'CONFIG', 'width=100,height=50,toolbar=no,status=no,resizable=no,top=20,left=100,dependent=yes,alwaysRaised=yes');
			}
		}
	</script>
	</head>
					...
					<table width="770" border="0" cellpadding="0" cellspacing="5">
						...
						<tr>
							...
							<td width="460" align="left" valign="top" bgcolor="#E6E6E6" class="conteudo" width="470">
								...
								<a href="javascript: void window.open('wizard_pagina_anunciante.php?modo=adiciona_imagem&passo=3&cd_pagina=<?=$cd_pagina?>', 'EVENTO', 'width=420,height=160,status=no,resizable=no,top=20,left=100,dependent=yes,alwaysRaised=yes');">[Adicionar Foto]</a>&nbsp;&nbsp;								
								<a href="javascript: apagar_evento(<?=$cd_evento?>);">[Apagar Pagina]</a>
								<hr>
								<iframe width="100%" height="500" src="wizard_pagina_anunciante.php?modo=update&cd_anunciante=<?=$codigo?>"></iframe>
							</td>
