...
function constroi_passo0(){
	...
	if($modo == "update"){
		...
		$query = "SELECT cd,  nome, descricao, site, email, telefone, endereco, tipo FROM anunciantes where cd=" . $codigo;
	...
	<html>
		...
		<body>
			<table>
				<form action="wizard_novo_anunciante.php" encType="multipart/form-data" method="post" name="sendform">
				...
				<tr>
					...
					<td>
						<select name="tipo">
						<?php
							$query = "SELECT * FROM tipodeanunciante ORDER BY tipo";
							require("../includes/conectar_mysql.php");
			...
			<iframe width="100%" src="form_tipodeanunciante.php" height="70" scrolling="no"></iframe>
		</body>
	</html>
	<? 
}
...
function constroi_passo1(){
	...
	if (($modo == "add") || ($altera_imagem == "sim")){
		if($altera_imagem == "sim") {
			$query = "SELECT path, path_thumb FROM anunciantes WHERE cd=" . $_POST["cd"];
			require("../includes/conectar_mysql.php");
			$result = mysql_query($query) or die("Erro ao atualizar registros no Banco de dados: " . mysql_error());
	...
	if ($modo == "add")	{
		$query = "INSERT INTO anunciantes (nome, descricao, site, email, telefone, endereco, tipo, path, path_thumb, largura, altura) VALUES ('";
		$query .= $nome ."', '";
		$query .= $descricao ."', '";
		$query .= $site ."', '";
		$query .= $email ."', '";
	...
	<html>
		<head>
			<title>Anunciante Salvo!</title>
		</head>
		<body>
			<h3>Anunciante Salvo com Sucesso!</h3>
			<br><br>
			<a href="wizard_novo_anunciante.php">[Adicionar Novo Anunciante]</a>
			<br><br>
