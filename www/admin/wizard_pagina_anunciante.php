...
function constroi_passo0(){
	$modo = $_REQUEST["modo"];
	$codigo = $_REQUEST["cd_anunciante"];
	
	if($modo == "update"){
		$update = true;
		require("../includes/conectar_mysql.php");
		$query = "SELECT * from pagina_anunciante where cd_parceiro=" . $codigo;
	...
	<html>
		<head>
			<title>Cadastro de anunciantes</title>
		...
		<body>
			<table width="100%">
				<form action="wizard_pagina_anunciante.php" method="post" name="paginaparceiro">
				<tr>
					<td colspan="2" class="label" style="text-align: center; font-weight: bold;">Texto da Pagina do anunciante</td>
				</tr>
				<tr>
				...
				<input type="hidden" name="cd_anunciante" value="<? if($update) echo($pagina["cd_parceiro"]); else echo($codigo);?>">
				</form>
			</table>
		</body>
		<script language="javascript" type="text/javascript">
...
function constroi_passo1(){
	...
	$modo = $_REQUEST["modo"];
	$cd = $_REQUEST["cd"];
	$cd_parceiro = $_REQUEST["cd_anunciante"];
	
	if ($modo == "add")	{
		$query = "INSERT INTO pagina_anunciante (cd_parceiro, texto) VALUES (";
		$query .= $cd_parceiro . ", '";
		$query .= $texto ."')";
	}
	if ($modo == "update") {
		$query = "UPDATE pagina_anunciante SET ";
	...
	<html>
		<head>
			<title>Pagina do anunciante: Segundo Passo</title>
			<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
		</head>
		<body>
		...
		<body>
			<table width='100%' border='0'>
				...
				if($modo != "adiciona_imagem"){ ?>
					<tr>
						<td align='center'>
							<form method="post" action="wizard_pagina_anunciante.php">
...
function constroi_passo3(){
	...
	if($_POST["numero_imagem"] == ""){
		require("../includes/conectar_mysql.php");
		$query = "SELECT cd FROM pagina_anunciante_fotos WHERE cd_pagina=" . $cd_pagina . " ORDER BY cd DESC";
		$result = mysql_query($query) or die("Erro ao atualizar registros no Banco de dados: " . $query .  mysql_error());
	...
	<html>
		<head>
			<title>Cadastro de anunciantes: Terceiro Passo</title>
			<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
		</head>
		<body>
			<table width="400" border="0" class="tabela">
